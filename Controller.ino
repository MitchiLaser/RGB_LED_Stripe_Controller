...
void loop( void ) {
  ...
  if (animation_running && (millis() - lastAnimPoll) > delayAnim) {
    lastAnimPoll = millis();
    animation();
    analogWrite(D2, animColors[0] * 4);
    analogWrite(D3, animColors[1] * 4);
    analogWrite(D1, animColors[2] * 4);
  }
}
...
void handlePower( void ) {
  ...
  analogWrite(D2, server.arg("r").toInt() * 4);
  analogWrite(D3, server.arg("g").toInt() * 4);
  analogWrite(D1, server.arg("b").toInt() * 4);
